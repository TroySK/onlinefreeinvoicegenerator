...
const translations = {
    en: {
        ...
        chooseTemplate: "Choose Template", modern: "Modern", classic: "Classic", professional: "Professional", elegant: "Elegant", nature: "Nature",
        ...
        invoiceHistory: "Invoice History", generatedByLabel: "Generated by OnlineFreeInvoiceGenerator.com", pageLabel: "Page"
    },
    es: {
        ...
        chooseTemplate: "Elegir Plantilla", modern: "Moderno", classic: "Clásico", professional: "Profesional", elegant: "Elegante", nature: "Naturaleza",
        ...
        invoiceHistory: "Historial de Facturas", generatedByLabel: "Generated by OnlineFreeInvoiceGenerator.com", pageLabel: "Página"
    },
    fr: {
        ...
        chooseTemplate: "Choisir le Modèle", modern: "Moderne", classic: "Classique", professional: "Professionnel", elegant: "Élégant", nature: "Nature",
        ...
        invoiceHistory: "Historique des Factures", generatedByLabel: "Generated by OnlineFreeInvoiceGenerator.com", pageLabel: "Page"
    },
    de: {
        ...
        chooseTemplate: "Vorlage Wählen", modern: "Modern", classic: "Klassisch", professional: "Professionell", elegant: "Elegant", nature: "Natur",
        ...
        invoiceHistory: "Rechnungsverlauf", generatedByLabel: "Generated by OnlineFreeInvoiceGenerator.com", pageLabel: "Seite"
    }
};
...
const templates = [
    {
        ...
        accentColor: "#a5b4fc",
        cssOverrides: "",
        layoutConfig: { showTaxColumn: true, discountPosition: "belowTotal", headerStyle: "bar", tableStyle: "striped" }
    },
    {
        templateId: "classic",
        ...
        accentColor: "#6b7280",
        cssOverrides: "",
        layoutConfig: { showTaxColumn: true, discountPosition: "belowTotal", headerStyle: "double-line", tableStyle: "bordered" }
    },
    {
        templateId: "business",
        name: "Professional",
        ...
        accentColor: "#93c5fd",
        cssOverrides: "",
        layoutConfig: { showTaxColumn: true, discountPosition: "belowTotal", headerStyle: "accent-bar", tableStyle: "striped" }
    },
    {
        templateId: "elegant",
        name: "Elegant",
...
function init() {
    ...
    function renderAllPDFPages() {
        ...
        _pdfDoc.getPage(1).then(function(page) {
            ...
            if (infoEl) infoEl.textContent = '1 / ' + numPages;
            var prevBtn = document.querySelector('.preview-prev');
            var nextBtn = document.querySelector('.preview-next');
            if (prevBtn) prevBtn.disabled = true;
            if (nextBtn) nextBtn.disabled = numPages <= 1;
            var promises = [];
            for (var j = 1; j <= numPages; j++) {
                promises.push(renderPDFPage(j));
            }
    ...
    function updateNavButtons() {
        ...
        var numPages = _pdfDoc.numPages;
        var infoEl = document.querySelector('.page-info');
        if (infoEl) infoEl.textContent = _pdfCurrPage + ' / ' + numPages;
        var prevBtn = document.querySelector('.preview-prev');
        var nextBtn = document.querySelector('.preview-next');
        if (prevBtn) prevBtn.disabled = _pdfCurrPage <= 1;
        if (nextBtn) nextBtn.disabled = _pdfCurrPage >= numPages;
        // Show only the current page
        var pages = document.querySelectorAll('.preview-page');
        pages.forEach(function(p, idx) {
            p.style.display = (idx + 1 === _pdfCurrPage) ? 'flex' : 'none';
        });
    }
    ...
    if (previewBtn) previewBtn.addEventListener('click', function() {
        ...
        // Show loading state
        container.innerHTML = '<div class="preview-loading"><div class="spinner"></div><span>Loading preview...</span></div>';
        ...
        try {
            ...
            pdfjsLib.getDocument(_pdfBlobUrl).promise.then(function(pdfDoc) {
                _pdfDoc = pdfDoc;
                _pdfScale = 1.5;
                renderAllPDFPages();
                updateNavButtons();
                ...
                container.innerHTML = '<div class="preview-loading">Failed to load preview.</div>';
            });
        } catch (e) {
    ...
    document.addEventListener('click', function(e) {
        var btn = e.target.closest('.preview-prev, .preview-next, .preview-zoom-in, .preview-zoom-out');
        if (!btn || !_pdfDoc) return;
        if (btn.classList.contains('preview-prev') && _pdfCurrPage > 1) {
            _pdfCurrPage--;
            updateNavButtons();
        } else if (btn.classList.contains('preview-next') && _pdfCurrPage < _pdfDoc.numPages) {
            _pdfCurrPage++;
            updateNavButtons();
        } else if (btn.classList.contains('preview-zoom-in')) {
            _pdfScale = Math.min(3, _pdfScale + 0.25);
            renderAllPDFPages();
            updateNavButtons();
            var label = document.querySelector('.zoom-label');
            if (label) label.textContent = Math.round(_pdfScale / 1.5 * 100) + '%';
        } else if (btn.classList.contains('preview-zoom-out')) {
...
function updateTemplate() {
    var validTemplates = ['modern1', 'classic', 'business', 'elegant', 'nature'];
    if (validTemplates.indexOf(invoiceData.template) === -1) {
        invoiceData.template = 'modern1';
    }

    // Remove all template classes
    invoiceContainer.classList.remove('template-modern1', 'template-classic', 'template-business', 'template-elegant', 'template-nature');
    ...
    tableHeaders.forEach(th => {
        // Remove all template classes
        th.classList.remove('template-modern1', 'template-classic', 'template-business', 'template-elegant', 'template-nature');
        // Add the selected template class
    ...
    const addItemBtn = document.getElementById('add-item-btn');
    // Remove all template classes
    addItemBtn.classList.remove('template-modern1', 'template-classic', 'template-business', 'template-elegant', 'template-nature');
    // Add the selected template class
    ...
    // Remove all template classes
    lineItemsTable.classList.remove('template-modern1', 'template-classic', 'template-business', 'template-elegant', 'template-nature');
    // Add the selected template class
...
function applyTranslations() {
    ...
    if (templateLabels[3]) templateLabels[3].textContent = t.elegant;
    if (templateLabels[4]) templateLabels[4].textContent = t.nature;
...
var PDF = {
    ...
    HEADER_BAR_Y: 0,
    TOP_CONTENT_Y: 18,
    INVOICE_TITLE_FONT_SIZE: 26,
    SECTION_LABEL_FONT_SIZE: 10,
    SECTION_TEXT_FONT_SIZE: 10,
    META_FONT_SIZE: 9,
    FROM_X: 20,
    TO_X: 105,
    BOX_WIDTH: 80,
    ...
    BOX_TEXT_Y_OFFSET: 16,
    ADDR_LINE_HEIGHT: 4.5,
    TABLE_FONT_SIZE: 9,
    TABLE_HEADER_FONT_SIZE: 9,
    TABLE_CELL_PADDING: 4,
    TOTALS_LABEL_X: 130,
    TOTALS_VALUE_X: 190,
    TOTALS_LINE_HEIGHT: 7,
    GRAND_TOTAL_FONT_SIZE: 12,
    NOTES_FONT_SIZE: 8,
    NOTES_LABEL_FONT_SIZE: 9,
...
function generatePDFContent(doc, t) {
    ...
    var secondaryColor = template ? template.secondaryColor : "#eef2ff";
    ...
    var lightColor = hexToRgb(secondaryColor);
    var currencyDisplay = getCurrencyDisplay(invoiceData.currency);
    t = t || translations[currentLang] || translations.en;
    var pageW = PDF.PAGE_WIDTH;
    var marginL = PDF.MARGIN_LEFT;
    var marginR = PDF.MARGIN_RIGHT;
    var defaultFont = "helvetica";

    doc.setFont(defaultFont);

    // ── HEADER BAR ──
    var isClassic = currentTemplate === "classic";
    if (isClassic) {
        doc.setDrawColor.apply(doc, headerColor);
        doc.setLineWidth(1.5);
        doc.line(marginL, 10, pageW - marginR, 10);
        doc.line(marginL, 13, pageW - marginR, 13);
        doc.setLineWidth(0.2);
    } else {
        var hasAccentBar = currentTemplate === "business" || currentTemplate === "nature";
        doc.setFillColor.apply(doc, headerColor);
        if (hasAccentBar) {
            doc.rect(0, 0, 4, PDF.PAGE_HEIGHT, 'F');
            doc.rect(0, 0, pageW, PDF.HEADER_BAR_HEIGHT, 'F');
        } else {
            doc.rect(0, 0, pageW, PDF.HEADER_BAR_HEIGHT, 'F');
        }
    }
    ...
    // ── LOGO ──
    var logoY = isClassic ? 22 : PDF.TOP_CONTENT_Y;
    var logoHeight = 0;
    if (invoiceData.logo) {
        try {
            var logoFormat = 'PNG';
            ...
            doc.addImage(invoiceData.logo, logoFormat, marginL, logoY, PDF.LOGO_MAX_WIDTH, PDF.LOGO_MAX_HEIGHT);
            logoHeight = PDF.LOGO_MAX_HEIGHT;
        } catch (e) {}
    }

    // ── TITLE & META (two-column top section) ──
    var titleTopY = logoHeight > 0 ? logoY + logoHeight + 6 : (isClassic ? 28 : PDF.TOP_CONTENT_Y + 8);
    var metaRightX = pageW - marginR;
    ...
    doc.text(invoiceTitle, marginL, titleTopY);
    ...
    if (invoiceData.status && invoiceData.status !== "") {
        ...
        var statusW = doc.getTextWidth(statusText) + 6;
        doc.setDrawColor.apply(doc, statusColor);
        doc.setFillColor(255, 255, 255);
        doc.roundedRect(metaRightX - statusW, titleTopY - 8, statusW, 11, 2, 2, 'FD');
        doc.setTextColor.apply(doc, statusColor);
        doc.text(statusText, metaRightX - 3, titleTopY, { align: 'right' });
        doc.setFont(defaultFont, 'normal');
    }

    // Meta info (right-aligned)
    doc.setFont(defaultFont, 'normal');
    doc.setFontSize(PDF.META_FONT_SIZE);
    doc.setTextColor(107, 114, 128);
    var metaY = titleTopY + 2;

    var metaLines = [];
    if (invoiceData.invoiceNumber) metaLines.push({ label: (t.invoiceNo || "Invoice #") + ":", value: invoiceData.invoiceNumber });
    if (invoiceData.date) metaLines.push({ label: (t.dateLabel || "Date") + ":", value: invoiceData.date });
    if (invoiceData.dueDate) metaLines.push({ label: (t.dueDateLabel || "Due Date") + ":", value: invoiceData.dueDate });
    if (invoiceData.poNumber.trim()) metaLines.push({ label: (t.poNumberLabel || "PO #") + ":", value: invoiceData.poNumber });

    metaLines.forEach(function(line, i) {
        var lineStr = line.label + " " + line.value;
        doc.text(lineStr, metaRightX, metaY + (i * 6), { align: 'right' });
    });

    var metaBlockHeight = metaLines.length * 6;
    var sectionStartY = Math.max(titleTopY + 14, metaY + metaBlockHeight + 6);

    // ── FROM / TO BOXES ──
    var boxLabelSize = PDF.SECTION_LABEL_FONT_SIZE;
    var boxTextSize = PDF.SECTION_TEXT_FONT_SIZE;
    var boxW = PDF.BOX_WIDTH;

    // From box
    var fromX = PDF.FROM_X;
    var toX = PDF.TO_X;
    var boxTopY = sectionStartY;

    var drawInfoBox = function(x, y, label, name, address, email) {
        var boxH = 32;
        doc.setFillColor.apply(doc, lightColor);
        doc.setDrawColor.apply(doc, headerColor);
        doc.setLineWidth(0.5);

        var nameLines = name ? 1 : 0;
        var addrLines = address ? doc.splitTextToSize(address, boxW - PDF.BOX_PADDING * 2).length : 0;
        var emailLines = email ? 1 : 0;
        var contentH = nameLines * PDF.ADDR_LINE_HEIGHT + addrLines * PDF.ADDR_LINE_HEIGHT + emailLines * PDF.ADDR_LINE_HEIGHT;
        boxH = Math.max(32, PDF.BOX_LABEL_Y_OFFSET + 3 + contentH + 8);

        doc.roundedRect(x, y, boxW, boxH, 2, 2, 'FD');

        doc.setFont(defaultFont, 'bold');
        doc.setFontSize(boxLabelSize);
        doc.setTextColor.apply(doc, headerColor);
        doc.text(label, x + PDF.BOX_PADDING, y + PDF.BOX_LABEL_Y_OFFSET + 3);

        doc.setFont(defaultFont, 'normal');
        doc.setFontSize(boxTextSize);
        doc.setTextColor(55, 65, 81);
        var textY = y + PDF.BOX_TEXT_Y_OFFSET + 3;
        doc.text(name || "", x + PDF.BOX_PADDING, textY);
        textY += PDF.ADDR_LINE_HEIGHT;
        if (address) {
            var addrLines = doc.splitTextToSize(address, boxW - PDF.BOX_PADDING * 2);
            doc.text(addrLines, x + PDF.BOX_PADDING, textY);
            textY += addrLines.length * PDF.ADDR_LINE_HEIGHT;
        }
        if (email) {
            doc.text(email, x + PDF.BOX_PADDING, textY);
        }
        return y + boxH + 8;
    };

    var afterBoxesY = drawInfoBox(fromX, boxTopY, (t.from || "FROM"), invoiceData.sender.name, invoiceData.sender.address, invoiceData.sender.email);
    drawInfoBox(toX, boxTopY, (t.to || "TO"), invoiceData.recipient.name, invoiceData.recipient.address, invoiceData.recipient.email);
    afterBoxesY = Math.max(afterBoxesY, boxTopY + 40);
    ...
    if (invoiceData.taxDetail.trim()) {
        ...
        doc.setFontSize(PDF.META_FONT_SIZE);
        doc.setTextColor(107, 114, 128);
        doc.text((t.taxDetailLabel || "Tax Detail:") + " " + invoiceData.taxDetail, marginL, afterBoxesY);
        afterBoxesY += 7;
    }

    // ── LINE ITEMS TABLE ──
    var tableStartY = afterBoxesY + 6;

    var tableHeaders = [[
        { content: t.description || "Description", styles: { halign: 'left' } },
        { content: t.quantity || "Qty", styles: { halign: 'center' } },
        { content: t.rate || "Rate", styles: { halign: 'right' } },
        { content: t.tax || "Tax %", styles: { halign: 'right' } },
        { content: t.amount || "Amount", styles: { halign: 'right' } }
    ]];

    var tableRows = invoiceData.lineItems.map(function(item, idx) {
        return [
            item.description,
            { content: String(item.quantity), styles: { halign: 'center' } },
            { content: currencyDisplay + item.rate.toFixed(2), styles: { halign: 'right' } },
            { content: item.tax + '%', styles: { halign: 'right' } },
            { content: currencyDisplay + item.amount.toFixed(2), styles: { halign: 'right', textColor: [55, 65, 81] } }
        ];
    });
    ...
    doc.autoTable({
        ...
        headStyles: {
            ...
            fontStyle: 'bold',
            fontSize: PDF.TABLE_HEADER_FONT_SIZE,
            cellPadding: PDF.TABLE_CELL_PADDING,
    ...
    var finalY = (doc.lastAutoTable && doc.lastAutoTable.finalY) || tableStartY + 40;

    // ── TOTALS SECTION ──
    var totalsY = finalY + 6;
    var labelX = PDF.TOTALS_LABEL_X;
    var valueX = PDF.TOTALS_VALUE_X;
    var lineH = PDF.TOTALS_LINE_HEIGHT;
    ...
    doc.setFont(defaultFont, 'normal');
    doc.setFontSize(PDF.SECTION_TEXT_FONT_SIZE);
    doc.setTextColor(107, 114, 128);
    var totalsData = [
        { label: t.subtotalLabel || 'Subtotal:', value: currencyDisplay + invoiceData.subtotal.toFixed(2) },
        { label: t.totalTaxLabel || 'Total Tax:', value: currencyDisplay + invoiceData.totalTax.toFixed(2) }
    ];
    if (invoiceData.discount > 0) {
        totalsData.push({ label: t.discountLabel || 'Discount:', value: '-' + currencyDisplay + invoiceData.discount.toFixed(2) });
    }

    totalsData.forEach(function(row, i) {
        var yPos = totalsY + 4 + (i + 1) * lineH;
        doc.text(row.label, labelX, yPos);
        doc.text(row.value, valueX, yPos, { align: 'right' });
    });

    // Grand Total
    var grandY = totalsY + 4 + (totalsData.length + 1) * lineH;
    doc.setDrawColor.apply(doc, headerColor);
    doc.line(labelX, grandY - 4, valueX, grandY - 4);

    doc.setFont(defaultFont, 'bold');
    doc.setFontSize(PDF.GRAND_TOTAL_FONT_SIZE);
    doc.setTextColor.apply(doc, headerColor);
    doc.text((t.grandTotalLabel || 'Grand Total:') + ' ', labelX, grandY + 2);
    doc.setFont(defaultFont, 'bold');
    doc.text(currencyDisplay + invoiceData.grandTotal.toFixed(2), valueX, grandY + 2, { align: 'right' });

    var currentY = grandY + 12;
    ...
    if (invoiceData.notes.trim()) {
        ...
        doc.text(t.notesLabel || "Notes / Terms:", marginL, currentY);
        doc.setFont(defaultFont, 'normal');
        doc.setFontSize(PDF.NOTES_FONT_SIZE);
        doc.setTextColor(107, 114, 128);
        var notesLines = doc.splitTextToSize(invoiceData.notes, PDF.NOTES_MAX_WIDTH);
        doc.text(notesLines, marginL, currentY + 5);
        currentY = currentY + 5 + (notesLines.length * 5) + 4;
    }
    ...
    if (invoiceData.paymentDetails && (invoiceData.paymentDetails.bank || invoiceData.paymentDetails.paypal || invoiceData.paymentDetails.link)) {
        ...
        doc.setFontSize(PDF.NOTES_LABEL_FONT_SIZE);
        doc.setTextColor.apply(doc, PDF.NOTES_TEXT_COLOR);
        var payLabel = t.paymentDetailsLabel || "Payment Details:";
        doc.text(payLabel, marginL, currentY);
        doc.setFont(defaultFont, 'normal');
        doc.setFontSize(PDF.NOTES_FONT_SIZE);
        doc.setTextColor(107, 114, 128);
        var payTextY = currentY + 5;
        if (invoiceData.paymentDetails.bank) { doc.text((t.bank || "Bank:") + " " + invoiceData.paymentDetails.bank, marginL, payTextY); payTextY += 5; }
        if (invoiceData.paymentDetails.paypal) { doc.text((t.paypal || "PayPal:") + " " + invoiceData.paymentDetails.paypal, marginL, payTextY); payTextY += 5; }
        if (invoiceData.paymentDetails.link) { doc.text((t.pay || "Pay:") + " " + invoiceData.paymentDetails.link, marginL, payTextY); payTextY += 5; }
        currentY = payTextY + 4;
    }
    ...
    doc.line(marginL, PDF.FOOTER_Y, pageW - marginR, PDF.FOOTER_Y);
    doc.setFont(defaultFont, 'normal');
    doc.setFontSize(PDF.FOOTER_FONT_SIZE);
    doc.setTextColor(156, 163, 175);
    if (!new URLSearchParams(window.location.search).has('no_footer')) {
        doc.text((t.generatedByLabel || "Generated by OnlineFreeInvoiceGenerator.com"), marginL, PDF.FOOTER_Y + 5);
    }
    doc.text((t.pageLabel || "Page") + " 1", pageW - marginR, PDF.FOOTER_Y + 5, { align: 'right' });
}
